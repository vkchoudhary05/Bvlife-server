...
export class ReviewService {
    ...
    createReview(params) {
        ...
        const hasDeliveredPurchase = db.getOrders().some(order => order.userEmail?.trim().toLowerCase() === normalizedEmail &&
        ...
        const existingReview = db.getReviews().some(review => review.productId === productId && review.userEmail?.trim().toLowerCase() === normalizedEmail);
        if (existingReview) {
