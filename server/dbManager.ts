...
class DBManager {
  ...
  private async syncWithMysql(): Promise<void> {
    try {
      ...
      if (mysqlUsers && mysqlUsers.length > 0) {
        this.data.users = mysqlUsers.map((u: any) => ({
          email: u.email,
          fullName: u.fullName,
          role: u.role,
      ...
      if (mysqlProducts && mysqlProducts.length > 0) {
        this.data.products = mysqlProducts.map((p: any) => ({
          id: p.id,
          name: p.name,
          sku: p.sku,
      ...
      if (mysqlOrders && mysqlOrders.length > 0) {
        this.data.orders = mysqlOrders.map((o: any) => ({
          id: o.id,
      ...
      if (mysqlReviews && mysqlReviews.length > 0) {
        this.data.reviews = mysqlReviews.map((r: any) => ({
          id: r.id,
      ...
      if (mysqlBlogs && mysqlBlogs.length > 0) {
        this.data.blogs = mysqlBlogs.map((b: any) => ({
          id: b.id,
          title: b.title,
          slug: b.slug,
          summary: b.summary || '',
      ...
      if (mysqlFAQs && mysqlFAQs.length > 0) {
        this.data.faqs = mysqlFAQs.map((f: any) => ({
          id: f.id,
          category: f.category,
          question: f.question,
          answer: f.answer
      ...
      if (mysqlLogs && mysqlLogs.length > 0) {
        this.data.activityLogs = mysqlLogs.map((l: any) => ({
          id: l.id,
      ...
      if (mysqlPayments && mysqlPayments.length > 0) {
        this.data.payments = mysqlPayments.map((p: any) => ({
          id: p.id,
          orderId: p.orderId,
  ...
  private async savePaymentToMysql(p: Payment): Promise<void> {
    const sql = `
  ...
  private async deleteProductFromMysql(id: string): Promise<void> {
    await query("DELETE FROM products WHERE id = ?", [id]);
  }

  private async saveUserToMysql(u: User): Promise<void> {
    const sql = `
  ...
  private async saveOrderToMysql(o: Order): Promise<void> {
    const sql = `
      INSERT INTO orders (
  ...
  private async saveReviewToMysql(r: Review): Promise<void> {
    const sql = `
  ...
  private async deleteReviewFromMysql(id: string): Promise<void> {
    await query("DELETE FROM reviews WHERE id = ?", [id]);
  }

  private async saveBlogToMysql(b: Blog): Promise<void> {
    const sql = `
  ...
  private async deleteBlogFromMysql(id: string): Promise<void> {
    await query("DELETE FROM blogs WHERE id = ?", [id]);
  }

  private async saveFAQToMysql(f: FAQ): Promise<void> {
    const sql = `
  ...
  private async deleteFAQFromMysql(id: string): Promise<void> {
    await query("DELETE FROM faqs WHERE id = ?", [id]);
  }

  private async saveCouponToMysql(c: Coupon): Promise<void> {
    const sql = `
  ...
  private async deleteCouponFromMysql(code: string): Promise<void> {
    await query("DELETE FROM coupons WHERE code = ?", [code.toUpperCase()]);
  }

  private async saveSettingsToMysql(s: WebsiteSettings): Promise<void> {
    const sql = `
  ...
  private async saveActivityLogToMysql(l: ActivityLog): Promise<void> {
    const sql = `
  ...
  saveSettings(settings: WebsiteSettings): WebsiteSettings {
  ...
  logActivity(userEmail: string, action: string, details: string) {
  ...
  getDoctorAppointmentsByUser(email: string): DoctorAppointment[] {
    const normalizedEmail = (email || '').toLowerCase().trim();
    return (this.data.doctorAppointments || []).filter(
      app => (app.patientEmail || '').toLowerCase().trim() === normalizedEmail
    );
  }

  bookDoctorAppointment(appointmentData: Partial<DoctorAppointment>): DoctorAppointment {
  ...
  updateDoctorAppointmentStatus(id: string, status: 'Confirmed' | 'Completed' | 'Cancelled'): DoctorAppointment | null {
    this.data.doctorAppointments = this.data.doctorAppointments || [];
  ...
  updateAppointmentWhatsAppStatus(id: string, sent: boolean = true): DoctorAppointment | null {
    this.data.doctorAppointments = this.data.doctorAppointments || [];
    const index = this.data.doctorAppointments.findIndex(a => a.id === id);
    if (index !== -1) {
  ...
  // --- CHAT HISTORIES ---
  getChatHistory(email: string): { role: 'user' | 'assistant'; content: string }[] {
    this.data.chatHistories = this.data.chatHistories || {};
    return this.data.chatHistories[email.toLowerCase()] || [];
  }

  saveChatHistory(email: string, messages: { role: 'user' | 'assistant'; content: string }[]) {
    this.data.chatHistories = this.data.chatHistories || {};
    this.data.chatHistories[email.toLowerCase()] = messages;
    this.save();
  }

  clearChatHistory(email: string) {
    this.data.chatHistories = this.data.chatHistories || {};
