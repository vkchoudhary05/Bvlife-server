...
import { Product, ProductVariant, Review, Blog, FAQ, Coupon } from "../types.js";
...
export const getProductById = (req: Request, res: Response) => {
  ...
  if (!rawProd) return res.status(404).json({ error: "Product not found." });
...
export const getProductVariant = (req: Request, res: Response) => {
  ...
  const variant = prod.variants?.find(v => v.id === variantId);
  if (!variant) return res.status(404).json({ error: "Variant not found on this product." });
