...
export class OrderService {
   ...
   */
  getOrders(user?: { email: string; role?: string }) {
    if (!user) {
   ...
   */
  getOrdersByUser(requestedEmail: string, currentUser?: { email: string; role?: string }) {
    if (!currentUser) {
  ...
  updateOrder(orderId: string, updates: {
    ...
    if (status) order.status = status as Order['status'];
    if (paymentStatus) order.paymentStatus = paymentStatus as Order['paymentStatus'];
    if (trackingNumber) order.trackingNumber = trackingNumber;

    if (status || comment) {
      const updateObj = {
        status: (status || order.status) as Order['status'],
        date: new Date().toISOString(),
   ...
   */
  trackOrder(rawIdentifier: string, reqUser?: { email: string; role?: string }) {
    if (!rawIdentifier) {
