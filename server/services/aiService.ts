...
export class AIService {
  /**
   * Run Ayurvedic Consultation
   */
  async consult(params: { symptoms?: string; bodyType?: string; lifestyle?: string; query?: string; lang?: string }) {
    const { symptoms, bodyType, lifestyle, query, lang } = params;
    if (!symptoms && !query) {
      throw { status: 400, message: "Please enter your symptoms, questions or select your body constitution details." };
    }
...
*अस्वीकरण (Disclaimer): ग्राम्स लाइफ आयुर्वेदिक अंतर्दृष्टि केवल पारंपरिक टिप्पणियां हैं। नैदानिक ​​उपचार के लिए कृपया अपने चिकित्सक से परामर्श लें।*
        `;
        return { consultation: fallbackResponseHi.trim() };
      }
...
`;
    ...
    return { consultation: response.text };
  }

  /**
   * Run Interactive Conversational AI Chat
   */
  async chat(messages: any[], lang?: string, userEmail?: string | null) {
    if (!messages || !Array.isArray(messages)) {
      throw { status: 400, message: "No chat history provided." };
    }
...
4. Emergency Check: If symptoms include emergency signs, advise urgent medical care.
5. Provide recommendation with official BV Life products only, foods, routine, yoga, and disclaimer.
...
`;
  ...
  /**
   * Chat History retrieval and clear
   */
  getChatHistory(email: string) {
    return db.getChatHistory(email);
  }

  clearChatHistory(email: string) {
    return db.clearChatHistory(email);
  }
}
